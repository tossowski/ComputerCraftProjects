...
blacklist = {[1]="enderstorage:ender_storage", [2]="computercraft:advanced_modem", [3]="minecraft:diamond_pickaxe", [4]="minecraft:coal_block"}
blockConfig = inv.getItemDict()
blocks = mio.loadModelFile("/models/treeModel.txt") 
startX, startY, startZ = -488, 73, -206
blockNum = 388408
colorCode = "{Frequency:{middle:0,left:9,right:0}}"
...
function getBlockRequest()
  ...
  while blockNum <= #blocks and
    ...
    if blockRequest[blockName] ~= nil then
      ...
      blockRequest[blockName] = {["amt"]=1, ["meta"]=0}
      slotsUsed = slotsUsed + 1
    end
    blockNum = blockNum + 1
...
function handleBlockPlace (x2,y2,z2,blockID)
  m.moveInXYZGrid(x2 + startX, y2 + startY + 1, z2 + startZ)
  ...
  blockConfig = inv.getItemDict()
  blockIndex = blockConfig[blockID .. "0"]
  turtle.digDown()
...
for i = blockNum, #blocks, 1 do
  ...
  meta = block[5]
  --print("X:" .. x .. " Y:" .. y .. " Z:" .. z)
  if blockNum ~= 466621 then
    handleBlockPlace(x1, y1, z1, blockName, 0)
  end
end
